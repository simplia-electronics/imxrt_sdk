...
status_t PHY_KSZ8081_Init(phy_handle_t *handle, const phy_config_t *config)
{
    ...
    if (result == kStatus_Success)
    {
        ...
        //result = MDIO_Write(handle->mdioHandle, handle->phyAddr, PHY_CONTROL2_REG, (data | PHY_CTL2_REFCLK_SELECT_MASK));
        if (result != kStatus_Success)
        {
            return result;
        }
            ...
            if (result == kStatus_Success)
            {
                ...
                while (counter--)
                {
                    ...
                    if (result == kStatus_Success)
                    {
                        ...
                        if (((bssReg & PHY_BSTATUS_AUTONEGCOMP_MASK) == PHY_BSTATUS_AUTONEGCOMP_MASK) && ((bssReg & PHY_BSTATUS_LINKSTATUS_MASK) == PHY_BSTATUS_LINKSTATUS_MASK))
                        {
...
status_t PHY_KSZ8081_GetLinkSpeedDuplex(phy_handle_t *handle, phy_speed_t *speed, phy_duplex_t *duplex)
{
    ...
    result = MDIO_Read(handle->mdioHandle, handle->phyAddr, PHY_BASICSTATUS_REG, &ctlReg);
    if (result == kStatus_Success)
    {
