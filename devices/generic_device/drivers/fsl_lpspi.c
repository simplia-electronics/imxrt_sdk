...
#define FSL_LPSPI_GPIO_PCS_PORT_CLICK_BOARD_0				GPIO5
#define FSL_LPSPI_GPIO_PCS_PIN_CLICK_BOARD_0				2U
#define FSL_LPSPI_GPIO_PCS_PORT_CLICK_BOARD_1				GPIO3
#define FSL_LPSPI_GPIO_PCS_PIN_CLICK_BOARD_1				25U
/*!
...
typedef enum _lpspi_pcs_clickboard_pol
{
	kLpspiClickBoardPolLow = 0,
	kLpspiClickBoardPolHigh = 1
} _lpspi_pcs_clickboard_pol;
...
static  _lpspi_pcs_clickboard_pol s_lpspiClickBoardPol;
static lpspi_which_pcs_t	s_lpspiClickBoardPcs;
...
static void _LPSPI_EnableChipSelect( _lpspi_pcs_clickboard_pol pol )
{
	if( s_lpspiClickBoardPcs == kLPSPI_Pcs_ClickBoard0 )
	{
		GPIO_PinWrite( FSL_LPSPI_GPIO_PCS_PORT_CLICK_BOARD_0, FSL_LPSPI_GPIO_PCS_PIN_CLICK_BOARD_0, pol );
	}
	else if (s_lpspiClickBoardPcs == kLPSPI_Pcs_ClickBoard1 )
	{
		GPIO_PinWrite( FSL_LPSPI_GPIO_PCS_PORT_CLICK_BOARD_1, FSL_LPSPI_GPIO_PCS_PIN_CLICK_BOARD_1, pol );
	}
}

static void _LPSPI_DisableChipSelect( _lpspi_pcs_clickboard_pol pol )
{
	if( s_lpspiClickBoardPcs == kLPSPI_Pcs_ClickBoard0 )
	{
		GPIO_PinWrite( FSL_LPSPI_GPIO_PCS_PORT_CLICK_BOARD_0, FSL_LPSPI_GPIO_PCS_PIN_CLICK_BOARD_0, !pol );
...
void LPSPI_MasterInit(LPSPI_Type *base, const lpspi_master_config_t *masterConfig, uint32_t srcClock_Hz)
{
    ...
    if( masterConfig->whichPcs < kLPSPI_Pcs_ClickBoard0 )
    {
    	/* Set specific PCS to active high or low */
		LPSPI_SetOnePcsPolarity(base, masterConfig->whichPcs, masterConfig->pcsActiveHighOrLow);
		s_lpspiClickBoardPol = masterConfig->pcsActiveHighOrLow;
		s_lpspiClickBoardPcs = masterConfig->whichPcs;
    }
    else
    {
    	/*TODO
    	 * - Init the GPIO and the MUX
    	 * - Set the polarization of the click board chipselect.
    	 * */

        gpio_pin_config_t _LPSPI_PCS_config = {
            .direction = kGPIO_DigitalOutput,
            .outputLogic = !(masterConfig->pcsActiveHighOrLow),
    	...
    	if( masterConfig->whichPcs == kLPSPI_Pcs_ClickBoard0 )
    	{
			...
			GPIO_PinInit(FSL_LPSPI_GPIO_PCS_PORT_CLICK_BOARD_0, FSL_LPSPI_GPIO_PCS_PIN_CLICK_BOARD_0, &_LPSPI_PCS_config );
    	}
    	else if ( masterConfig->whichPcs == kLPSPI_Pcs_ClickBoard1 )
    	{
			IOMUXC_SetPinMux( IOMUXC_GPIO_EMC_39_GPIO3_IO25, 0U);
			IOMUXC_SetPinConfig( IOMUXC_GPIO_EMC_39_GPIO3_IO25, 0x10B0U );
			GPIO_PinInit(FSL_LPSPI_GPIO_PCS_PORT_CLICK_BOARD_1, FSL_LPSPI_GPIO_PCS_PIN_CLICK_BOARD_1, &_LPSPI_PCS_config );
    	}

    	LPSPI_SetOnePcsPolarity(base, kLPSPI_Pcs0, masterConfig->pcsActiveHighOrLow);
    	s_lpspiClickBoardPol = masterConfig->pcsActiveHighOrLow;
    	s_lpspiClickBoardPcs = masterConfig->whichPcs;
    }
    ...
    else
    {
        ...
        base->TCR = LPSPI_TCR_CPOL(masterConfig->cpol) | LPSPI_TCR_CPHA(masterConfig->cpha) |
                    LPSPI_TCR_LSBF(masterConfig->direction) | LPSPI_TCR_FRAMESZ(masterConfig->bitsPerFrame - 1U) |
                    LPSPI_TCR_PRESCALE(tcrPrescaleValue) | LPSPI_TCR_PCS( kLPSPI_Pcs0 );
    }
...
void LPSPI_SlaveGetDefaultConfig(lpspi_slave_config_t *slaveConfig)
{
    ...
    slaveConfig->pcsActiveHighOrLow = kLPSPI_PcsActiveHigh; /*!< Disable de PCS to be always assert, also the PCS pin has to be not select in the pin muxing. */
...
static void _LPSPI_GetChipSelect( LPSPI_Type *base, GPIO_Type* gpio, uint8_t* pin )
{
	uint32_t  reg = (base->TCR & LPSPI_TCR_PCS_MASK);
...
status_t LPSPI_MasterTransferBlocking(LPSPI_Type *base, lpspi_transfer_t *transfer)
{
    ...
    if( whichPcs >= kLPSPI_Pcs_ClickBoard0 )
    {
    	base->TCR =
    	        (base->TCR & ~(LPSPI_TCR_CONT_MASK | LPSPI_TCR_CONTC_MASK | LPSPI_TCR_RXMSK_MASK | LPSPI_TCR_PCS_MASK)) |
    	        LPSPI_TCR_CONT(isPcsContinuous) | LPSPI_TCR_CONTC(0) | LPSPI_TCR_RXMSK(isRxMask) | LPSPI_TCR_PCS(kLPSPI_Pcs0);
    }
    else
    {
    	base->TCR =
    ...
    _LPSPI_EnableChipSelect(s_lpspiClickBoardPol);
    if (bytesPerFrame <= 4U)
    {
    ...
    while (txRemainingByteCount > 0U)
    {
        ...
        if (waitTimes == 0U)
        {
        	_LPSPI_DisableChipSelect(s_lpspiClickBoardPol);
